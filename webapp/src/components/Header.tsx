...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-white border-b shadow-sm flex items-center justify-between px-6 py-3 relative">
      ...
      {menuOpen && (
        <div className="absolute top-full left-0 w-full bg-white border-b shadow-md flex flex-col gap-2 px-6 py-4 md:hidden animate-fade-in z-50">
          ...
          <div className="flex flex-col gap-2 mt-4">
            <button className="border border-gray-300 rounded-full px-4 py-1 font-semibold hover:bg-gray-100 transition w-full">Log in</button>
            <button className="bg-[#4ccfc1] text-white rounded-full px-5 py-1 font-semibold hover:bg-[#3bb9a0] transition w-full">Get started</button>
          </div>
        </div>
      )}
    </header>
